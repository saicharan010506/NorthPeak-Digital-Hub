...
export function Services() {
  return (
    <section id="services" className="np-lazy-section np-section bg-[var(--np-bg)] text-[var(--np-white)]">
      <div className="np-container">
        <div className="mb-14 grid gap-6 md:grid-cols-[1fr_340px] md:items-end">
          <div>
            <p className="np-eyebrow mb-4">Capabilities</p>
            <h2 className="np-display max-w-2xl text-5xl font-extrabold leading-[.96] tracking-[-.065em] md:text-7xl">Everything your <span className="text-[var(--np-purple)]">next move</span> needs.</h2>
          </div>
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {services.map(([number, title, description, Icon], i) => (
            <m.article
              ...
              className="group relative min-h-[270px] overflow-hidden rounded-3xl border border-[var(--np-soft-line)] bg-[var(--np-surface)] p-7 transition-all duration-300 hover:-translate-y-1 hover:border-[var(--np-violet)]/60 hover:shadow-[0_18px_60px_rgba(109,93,254,.11)] md:p-8"
            >
              <div className="absolute -right-16 -top-16 h-36 w-36 rounded-full bg-[var(--np-violet)]/10 opacity-0 blur-3xl transition-opacity duration-300 group-hover:opacity-100" aria-hidden="true" />
              <div className="relative flex items-start justify-between">
                <span className="text-xs font-semibold text-[var(--np-purple)]">{number}</span>
                <Icon aria-hidden="true" className="h-8 w-8 stroke-[1.3] text-[var(--np-purple)] transition-transform duration-300 group-hover:rotate-6 group-hover:scale-110" />
              </div>
