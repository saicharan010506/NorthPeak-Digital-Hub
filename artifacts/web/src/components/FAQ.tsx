...
export function FAQ() {
  ...
  return <section id="faq" className="np-lazy-section np-section bg-[var(--np-surface)] text-[var(--np-white)]"><div className="np-container grid gap-12 lg:grid-cols-[.75fr_1.25fr]"><div><p className="np-eyebrow mb-4">Questions, answered</p><h2 className="np-display text-5xl font-extrabold leading-[.96] tracking-[-.07em] md:text-6xl">No mystery, <span className="text-[var(--np-purple)]">just clarity.</span></h2></div><div className="divide-y divide-[var(--np-line)] border-y border-[var(--np-line)]">{faqs.map(([question, answer], i) => <div key={question}><button id={`faq-question-${i}`} data-testid={`button-faq-${i}`} type="button" aria-expanded={active === i} aria-controls={`faq-answer-${i}`} onClick={() => setActive(active === i ? null : i)} className="np-outline-focus flex w-full items-center justify-between gap-6 rounded py-6 text-left text-base font-semibold"><span>{question}</span><ChevronDown aria-hidden="true" className={`h-5 w-5 shrink-0 text-[var(--np-purple)] transition-transform duration-300 ${active === i ? 'rotate-180' : ''}`} /></button><div id={`faq-answer-${i}`} role="region" aria-labelledby={`faq-question-${i}`} className={`grid transition-[grid-template-rows,opacity] duration-300 ${active === i ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}><p className="overflow-hidden pb-6 pr-10 text-sm leading-relaxed text-[var(--np-slate)]">{answer}</p></div></div>)}</div></div></section>;
}