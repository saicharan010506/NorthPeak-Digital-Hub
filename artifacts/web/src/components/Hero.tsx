...
export function Hero() {
  ...
  return (
    <section id="home" onMouseMove={handleMove} className="relative min-h-[760px] overflow-hidden bg-[var(--np-bg)] text-[var(--np-white)]">
      {/* Decorative background grid — hidden from assistive tech */}
      <div aria-hidden="true" className="absolute inset-0 np-grid opacity-70" />
      {/* Decorative ambient glows */}
      <div aria-hidden="true" className="np-pulse pointer-events-none absolute -left-40 top-20 h-[420px] w-[420px] rounded-full bg-[var(--np-violet)]/20 blur-[110px]" />
      <div aria-hidden="true" className="np-pulse pointer-events-none absolute -right-24 top-44 h-[360px] w-[360px] rounded-full bg-[var(--np-purple)]/15 blur-[110px]" style={{ animationDelay: '1.5s' }} />
      <div className="relative z-10 mx-auto flex min-h-[760px] max-w-[1240px] flex-col px-5 pb-8 pt-5 md:px-10 md:pt-7 lg:px-14">
        <nav className="flex items-center justify-between" aria-label="Main navigation">
          <a href="#home" data-testid="link-logo" className="np-outline-focus flex items-center gap-2.5 rounded-lg text-sm font-bold tracking-[-.02em]">
            <span className="np-logo-mark" aria-hidden="true">N</span><span>NorthPeak <span className="text-[var(--np-purple)]">Digital</span></span>
          </a>
          <div className="hidden items-center gap-7 text-xs text-[var(--np-slate)] lg:flex">
            {links.map(([label, href]) => (
              <a key={href} data-testid={`link-nav-${label.toLowerCase()}`} className="np-outline-focus rounded-md transition-colors hover:text-[var(--np-white)]" href={href}>{label}</a>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <a href="#contact" data-testid="link-nav-cta" className="np-outline-focus hidden rounded-full bg-[var(--np-white)] px-4 py-2.5 text-xs font-bold text-[var(--np-bg)] transition-transform hover:-translate-y-0.5 sm:block">Let&apos;s talk <ArrowRight aria-hidden="true" className="ml-1 inline h-3.5 w-3.5" /></a>
            <button type="button" data-testid="button-mobile-menu" aria-label={open ? 'Close navigation menu' : 'Open navigation menu'} aria-expanded={open} onClick={() => setOpen(!open)} className="np-outline-focus rounded-lg border border-[var(--np-line)] p-2.5 lg:hidden">
        ...
        <div className="grid flex-1 items-center gap-12 pb-14 pt-24 lg:grid-cols-[1.04fr_.96fr] lg:gap-10 lg:pt-28">
          ...
          >
            ...
            <div className="absolute left-1/2 top-1/2 grid h-24 w-24 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-3xl border border-white/15 bg-white/[.08] shadow-[0_20px_60px_rgba(109,93,254,.4)] backdrop-blur-xl">
              <Sparkles className="h-8 w-8 text-[var(--np-white)]" />
            </div>
            <div className="np-float absolute left-[2%] top-[20%] w-44 rounded-2xl border border-white/10 bg-[var(--np-surface)]/90 p-4 shadow-2xl backdrop-blur-xl" style={{ animationDelay: '-2s' }}>
              <div className="flex items-center justify-between text-[10px] text-[var(--np-slate)]"><span>Conversion rate</span><TrendingUp className="h-3.5 w-3.5 text-[var(--np-purple)]" /></div>
              <strong className="np-display mt-2 block text-2xl">+38.4%</strong>
              <div className="mt-3 h-1 overflow-hidden rounded-full bg-white/10"><div className="h-full w-[78%] rounded-full bg-[var(--np-purple)]" /></div>
            </div>
            <div className="np-float absolute bottom-[14%] right-[-2%] flex items-center gap-3 rounded-2xl border border-white/10 bg-[var(--np-surface)]/90 px-4 py-3 shadow-2xl backdrop-blur-xl" style={{ animationDelay: '-4s' }}>
              <span className="grid h-8 w-8 place-items-center rounded-xl bg-[var(--np-violet)]/20"><MousePointer2 className="h-4 w-4 text-[var(--np-purple)]" /></span>
              <span><strong className="block text-xs">Experience, refined</strong><small className="text-[10px] text-[var(--np-slate)]">Built to be remembered</small></span>
            </div>
          </m.div>
        </div>
        <a data-testid="link-scroll-indicator" href="#trusted" className="np-outline-focus flex w-fit items-center gap-3 rounded text-[10px] uppercase tracking-[.2em] text-[var(--np-slate)]">
          <span className="grid h-8 w-5 place-items-center rounded-full border border-[var(--np-line)]"><ArrowDown aria-hidden="true" className="h-3 w-3 motion-safe:animate-bounce" /></span> Scroll to explore
        </a>
      </div>
    </section>
  );
